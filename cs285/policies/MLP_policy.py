...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray, mode='eval') -> Union[np.ndarray, distributions.Distribution]:
        ...
        if mode == 'eval':
            model.eval()
            return self(observation).sample().detach().cpu().numpy()
        elif mode == 'train':
            model.train()
...
class MLPPolicyAC(MLPPolicy):
    ...
    def update(self, observations, actions, adv_n=None):
        ...
        pred: distributions.Distribution = self.get_action(observations, mode='train')
        loss = -(pred.log_prob(actions) * adv_n).sum()
        loss.backward()
        self.optimizer.step()
